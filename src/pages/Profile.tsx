...
export default function Profile() {
  ...
  function calculatePayroll(baseRate: number) {
    ...
    for (let i = 0; i < brackets.length; i++) {
      const chunk = Math.min(brackets[i], gross) - (i === 0 ? 0 : brackets[i - 1]);
      tax += chunk * rates[i];
      gross -= chunk;
      if (gross <= 0) break;
    }
    // Final bracket for remaining income
    if (gross > 0) tax += gross * rates[rates.length - 1];
    ...
    <div className="p-6 max-w-3xl bg-white shadow-xl rounded-xl mx-auto my-8" ref={ref}>
      ...
      <div className="mb-8 p-4 border border-indigo-100 bg-indigo-50 rounded-lg">
        ...
        <ul className="space-y-3">
          {skillScores.map((s) => (
            <li key={s.name} className="flex justify-between item-center bg-white p-3 rounded-md shadow-sm">
      ...
      <div className="flex justify-center border-t pt-4">
        <div>
          <h3 className="text-xl font-semibold text-green-600 font-bold mb-2">Highest Scored Skill</h3>
          <div className="text-lg text-green-600 font-bold">{bestSkill ? `${bestSkill.name} ($${bestSkill.score.toFixed(3)})` : 'N/A'}</div>
        </div>
        <button
          onClick={() => setShowAdvanced(!showAdvanced)}
          className="text-sm text-indigo-500 hover:text-indigo-700 transition duration-150 m-4"
        >
      ...
      {showAdvanced && (
        <div className="mt-6 p-4 border border-gray-200 rounded-lg bg-gray-100">
          <h3 className="text-lg font-bold mb-3">Advanced Analytics (Dummy/Unused)</h3>
          <div className="space-y-3">
            <div>
              <span className="font-medium">Complex Transform Value (hours logged): </span>
              {complexTransform(hoursLogged).toFixed(2)}
            </div>
            <div>
              <span className="font-medium">Simulated Future Earnings (Unused): </span>
              {simulateFutureEarnings(2000).toFixed(2)}
            </div>
            <div>
              <button
                onClick={() => randomizeSkills()}
                className="border p-1 text-xs rounded hover:bg-gray-200"
              >
                Randomize Skill Levels
              </button>
              <button
                onClick={() => setDummyState(dummyState + 1)}
                className="border p-1 text-xs rounded ml-2"
              >
